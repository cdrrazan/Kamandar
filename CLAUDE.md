...
ruby lib/kamandar.rb --menubar      # print a SwiftBar/xbar plugin document (one shot, stdout)
...
./menubar/install-menubar.sh [5m]   # install as a SwiftBar menu-bar plugin (needs SwiftBar)
./menubar/uninstall-menubar.sh      # remove the SwiftBar plugin
```
...
- **`Surface` / `TerminalSurface` / `DashboardSurface` / `BrowserSurface` / `ServerSurface` / `MenubarSurface`** — consume buckets only; never re-query or re-classify. Contract is `render`/`page(buckets, ...) -> String` + `emit`. Adding email = another new surface, **no engine change**. `ServerSurface` reuses `BrowserSurface`'s `css`/`card`/`sections_html` and adds an in-page scope control bar. `MenubarSurface` (`--menubar`) is the proof of that extensibility: it emits a **SwiftBar/xbar plugin document** to stdout (first line = bar title, `---` = dropdown, `--` = submenu item, ` | href= color=` = params), pure and token-free; `menubar/` ships a plugin-wrapper template + install/uninstall scripts that render it into the SwiftBar plugin folder (interval in the filename). SwiftBar is an external app (like cloudflared), not a Ruby dep.
